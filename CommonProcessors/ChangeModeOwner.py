...
class ChangeModeOwner(Processor):
	...
	def main(self):
        ...
        if mode:
    		retcode = subprocess.call(['/bin/chmod', '-R', mode, resource_path])
    		if retcode:
    			raise ProcessorError('Error setting mode (chmod -R {}) for {}'.format(mode, resource_path))

        if owner:
    		retcode = subprocess.call(['/usr/sbin/chown', '-R', owner, resource_path])
    		if retcode:
    			raise ProcessorError('Error setting user ownership (chown -R {}) for {}'.format(owner, resource_path))

        if group:
    		retcode = subprocess.call(['/usr/bin/chgrp', '-R', group, resource_path])
    		if retcode:
