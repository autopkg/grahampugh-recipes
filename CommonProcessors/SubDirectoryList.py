...
class SubDirectoryList(Processor):
    ...
    input_variables = {
        ...
        'LANGUAGE': {
            "required": False,
            "description": ("Language of the pkg or DMG containing PKG,"
                            "one of ML, DE, EN."),
            'default': '',
        },
        'LICENSE': {
            "required": False,
            "description": ("License of the pkg or DMG containing PKG,"
                            "one of Floating, Node."),
            'default': '',
        },
        'EXCEPTION': {
            "required": False,
    ...
    def main(self):
        ...
        dmg_language = '%s' % self.env['LANGUAGE']
        dmg_license = '%s' % self.env['LICENSE']
        ...
        for dirName, subdirList, fileList in self.walk(self.env['root_path'], int(self.env['max_depth'])):
            ...
            for fname in fileList:
                if '.DS_Store' in fname:
                    continue
                relpath = os.path.relpath(os.path.join(fname, dirName), self.env['root_path'])
